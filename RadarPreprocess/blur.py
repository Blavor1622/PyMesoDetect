...
def get_echo_list(img_path, target_color):
    ...
    for x in range(consts.radar_area[0], consts.radar_area[1]):
        for y in range(consts.radar_area[0], consts.radar_area[1]):
            ...
            if consts.is_color_equal(pixel_value, target_color, 10):
...
def get_layer_model(filled_img_path):
    ...
    for x in range(consts.radar_area[0], consts.radar_area[1]):
        for y in range(consts.radar_area[0], consts.radar_area[1]):
            ...
            for scale_index in range(len(consts.negative_scale)):
                if consts.is_color_equal(pixel_value, consts.negative_scale[scale_index], 10):
            ...
            for scale_index in range(len(consts.positive_scale)):
                if consts.is_color_equal(pixel_value, consts.positive_scale[scale_index], 10):
...
def get_merged_img(folder_path, layer_model, scale_index):
    ...
    # check scale_index
    if scale_index < 0 or scale_index >= len(consts.color_velocity_pairs):
    ...
    merged_img = Image.open(consts.v_base_path)
...
def get_blur_including_component(folder_path, filled_img_path, layer_model, biggest_component, scale_index):
    ...
    for component in merged_target_echo_components:
        for coordinate in component:
            test_draw.point(coordinate, consts.color_velocity_pairs[scale_index][0])
...
def get_surrounding_echo_list(filled_img_path, enclosure):
    ...
    for echo_coordinate in enclosure:
        ...
        if (consts.is_color_equal(top_value, bottom_value, 10)
                and consts.is_color_equal(left_value, right_value, 10)
                and consts.is_color_equal(top_value, right_value, 10)):
            continue
        ...
        if (not consts.is_color_equal(top_value, self_value, 10)
                and not consts.is_color_equal(top_value, (0, 0, 0), 10)):
            if top_coordinate not in surrounding:
                surrounding.add(top_coordinate)
        if (not consts.is_color_equal(bottom_value, self_value, 10)
                and not consts.is_color_equal(bottom_value, (0, 0, 0), 10)):
            if bottom_coordinate not in surrounding:
                surrounding.add(bottom_coordinate)
        if (not consts.is_color_equal(left_value, self_value, 10)
                and not consts.is_color_equal(left_value, (0, 0, 0), 10)):
            if left_coordinate not in surrounding:
                surrounding.add(left_coordinate)
        if (not consts.is_color_equal(right_value, self_value, 10)
                and not consts.is_color_equal(right_value, (0, 0, 0), 10)):
...
def calculate_average_shear(filled_img_path, surrounding, enclosure_instance):
    ...
    for c_v_pair in consts.color_velocity_pairs:
        if consts.is_color_equal(instance_color, c_v_pair[0], 10):
    ...
    for coordinate in surrounding:
        ...
        for c_v_pair in consts.color_velocity_pairs:
            if consts.is_color_equal(echo_color, c_v_pair[0], 10):
...
def replace_blur_echo(filled_img_path, target_components, rm_img, filled_img, mode):
    ...
    for enclosure in target_components:
        ...
        if average_shear >= consts.blur_threshold:
            # iterate each echo within the enclosure
            for coordinate in enclosure:
                if mode == 'neg':
                    rm_draw.point(coordinate, consts.negative_scale[6])
                    filled_draw.point(coordinate, consts.negative_scale[6])
                else:
                    rm_draw.point(coordinate, consts.positive_scale[6])
                    filled_draw.point(coordinate, consts.positive_scale[6])
...
def remove_velocity_blur(folder_path, filled_img_path):
    ...
    # create result image
    rm_img = Image.open(consts.v_base_path)
    filled_img = Image.open(filled_img_path)
